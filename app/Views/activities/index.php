...
$userActions    = count(array_filter($activities, fn($a) => stristr($a['action'], 'USER') !== false));
$salesActions   = count(array_filter($activities, fn($a) => stristr($a['action'], 'SALE') !== false || stristr($a['action'], 'ITEM') !== false));
$systemActions  = count(array_filter($activities, fn($a) => stristr($a['action'], 'LOGIN') !== false || stristr($a['action'], 'AUTH') !== false));
?>
                ...
                <?php else: ?>
                    <?php foreach ($activities as $idx => $act): ?>
                        <tr>
                            ...
                            <td><span class="badge-action"><?= strtoupper(htmlspecialchars($act['action'])) ?></span></td>
                            <td><?= htmlspecialchars($act['description']) ?></td>
                            <td><code><?= htmlspecialchars($act['ip_address'] ?? '127.0.0.1') ?></code></td>
                            <td><?= date('d M, Y h:i A', strtotime($act['created_at'])) ?></td>
                        </tr>
                    <?php endforeach; ?>
                <?php endif; ?>
            </tbody>
